...
## Gotchas
- **Python env**: system `/usr/bin/python` lacks numpy. Use `python` (or `.venv/bin/pytest`) — there are venvs at both `.venv` and `.venv`. The relevant one for worktrees under `experiments/phase2-utility/worktrees/` is the latter.
